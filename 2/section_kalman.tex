...
\]
ここで $v_k\sim\mathcal N(0,\sigma_v^2)$，$w_k\sim\mathcal N(0,\sigma_w^2)$ は互いに独立で白色，$a_k,c_k$ は既知とする。目的は「時刻 $k$ までの観測 $\{y_\ell\}_{\ell=1}^k$ に基づく $\theta_k$ の最小平均二乗誤差（MMSE）推定」を行うこと\cite{exp2025}.
...
\paragraph{考察}
同じシード値でカルマンフィルタ,カルマンスムーサーを実行したが,図\ref{fig:task11_fig}と図\ref{fig:task12_fig}を比較しても,mmseを比較しても,カルマンスムーサーの方が真値に近い推定ができていることがわかる.これは,カルマンスムーサーは全データを用いて各時刻の状態を推定するため,より多くの情報を活用できることによると考えられる.
