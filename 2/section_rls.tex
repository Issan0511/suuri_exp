...
\paragraph{考察}
ブロックごとに異なる重み（$Q_{6000}\neq Q_{4000}$）を用いた合成と，全体に単一重みをかけた WLS は厳密には一致しないが，差は十分小さい。微小な差が生じる原因として,各ブロックでの分散推定 $V$ のばらつきが考えられる。
...
\]
$w_k\sim\mathrm{Uniform}[-1,1]$。RLS 更新は資料の式(3.26) ~ (3.28)に従う（忘却なし $\gamma=1$）\cite{exp2025}。
...
定数入力で効果が極小かつ雑音が大きく，bの寄与は雑音に埋もれる。。
...
\paragraph{考察}
$F$ は時間変化するが振幅が小さく，$b$ は依然として真値より二桁以上大きい。




\subsubsection*{(3) 入力 $F_k=1000000\sin(\pi k/5)$（大振幅正弦）}
\paragraph{内容}
入力振幅を $10^6$ 倍して強い励起を与える。\cite{exp2025}
...
\paragraph{考察}
$b$ が真値 $5.0\times10^{-5}$ に一致。十分な振幅で雑音が相対的に非常に小さくなり，$F$ の係数が正しく同定できる。$a_1,a_2$ も誤差が小さい。
...
\paragraph{考察}
図\ref{fig:task10}から確かに，$ \theta $の推定が真の値に追随していることが分かる。忘却係数を導入することで，過去のデータの影響を減少させ，最新のデータに基づく推定が可能となっている。
