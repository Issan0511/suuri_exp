...
\end{align}
で与えられる\cite{exp2025}．初期点は $x_0=1/2$ とする．停止判定は $|f'(x_k)|\le \varepsilon$（$\varepsilon=10^{-8}$）とし，最大反復回数も設ける．
...
\end{align}
で与えられる\cite{exp2025}．初期点は $x_0=5$ とし，(a) と同様に $|f'(x_k)|\le \varepsilon$ を停止判定とした．なお，本問題では $f''(x)=0$（すなわち $x=1$）で更新が不能となるため，実装では $f''(x_k)=0$ の場合を例外として扱う．
